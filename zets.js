...
//Katalog
const katalog = (teks) => {
             res = marubotz.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 111119999, "message": teks, "footerText": "*ZetsBotz*", "thumbnail": thumb, "surface": 'CATALOG' }}, {quoted:mek})
             marubotz.relayWAMessage(res)
        }
...
const { wa_version, os_version } = marubotz.user.phone
...
├ *Owner Number:* @${pemilik.split('@')[0]}
├ *Host Name:* ${os.hostname()}
├ *Platform:* ${os.platform()}
├ *Wa Version:* ${marubotz.user.phone.wa_version}
├ *Mode:* ${self ? "Self" : "Public"}
├ *Autoread:* ${autoread ? "Active" : "Off"}
├ *Autotype:* ${autoketik ? "Active" : "Off"}
├ *Autovn:* ${autovn ? "Active" : "Off"}
└
...
└`
but = [
{ buttonId: `${prefix}allmenu`, buttonText: { displayText: 'MENU BOT️' }, type: 1 },
...
sendButImage(from, menu, `*_${tanggal} - ${time}_*`, fakeimage, but, {quoted: ftrol})
break
...
gambar = await getBuffer(`https://telegra.ph/file/80689010b78f902b7cfe7.jpg`)
menunya = `*「 PAYMENT 」*

• Qris : Scan kode qr di atas, isi nominal dan bayar
• Dana/Gopay : 081252848955 
`
but = [
{ buttonId: `${prefix}menu`, buttonText: { displayText: 'OWNER️' }, type: 1 }
]
sendButImage(from, menunya, `*_${tanggal} - ${time}_*`, gambar, but)
break

case 'rules':
reply(`1. Jangan spam bot. 
...
reply(mess.wait)
lah = await fetchJson(`https://api.zekais.com/mediafire?url=${q}&apikey=zekais`)
down = await getBuffer(lah.result.url)
marubotz.sendMessage(from, down, document, { mimetype: 'application/zip', filename: lah.result.size, quoted: mek, contextInfo: {mentionedJid: [sender]}})
break
